export default {
  ...
  multiplierBreakdown: {
    ...
    electromagneticPowerGainNote: "此栏显示粒子每次碰到判定线获得的 EP。碰线频率、EP 衰减和触碰边界的惩罚均为独立机制，不计入这里。",
    base: "基础输入",
    totalFactor: "总倍率",
    final: "最终产出",
    source: "来源",
    ...
    entries: {
      ...
      "ep-power-gain-upgrade": "EP 产量升级",
      "ep-line-gain-upgrade": "碰线 EP 升级",
      "coherence-ep-gain": "调谐 EP 升级",
    },
  },
  mainPage: {
    title: "梦能增量",
  ...
  conceptCrystals: {
    ...
    togglePathTitle: "点击切断或恢复 {from} 到 {to} 的生产",
    toggleInnerProductionTitle: "点击冻结或恢复{concept}的生产",
    innerFrozen: "已冻结",
    innerNetProduction: "净产量 {value}/turn",
    innerCycleLabel: "循环压制",
    ...
    effect: {
      title: "当前概念水晶效果",
      copy: "下方展示每个概念推动或压制对应机制的力度。相对的概念会彼此抵消，上方显示它们共同作用后的实际影响；持有更多概念水晶会放大这些影响。",
      copyCrushTwo: "共识取代了征服，不再压制 CP 获取，并独立提升 Shielding Efficiency。下方展示各概念的影响力度，上方显示它们共同作用后的实际影响。",
      copyCrushFour: "自由取代了桎梏，不再加剧兼并，并独立削弱前四个 DE Softcap。下方展示各概念的影响力度，上方显示它们共同作用后的实际影响。",
      copyCrushFive: "信念、公正与革新构成内圈循环。每个概念由相邻的两个外圈概念生产，并会从下一个内圈概念的生产中减去自身数量。点击流入内圈的生产线即可冻结或恢复对应概念。",
      copyCrushSix: "和平取代了战火，不再加剧 DC 价格增长，并温和地加快整个概念回路；在当前节点上限附近约为 ×3，突破节点上限后仍可继续增长。",
      dcCostGrowth: "概念对 20 个后 DC 价格增长的实际影响：×{value}",
      cpGain: "概念对 CP 获取的实际影响：×{value}",
      assimilation: "概念对兼并强度的实际影响：×{value}",
      shieldingEfficiency: "共识提供的 Shielding Efficiency：×{value}",
      softcapEfficiency: "自由提供的前四个 DE Softcap Efficiency：×{value}",
      conceptProductionSpeed: "和平提供的概念生产速度：×{value}",
      faith: "信念使所有生效中的角色加成提升至 ^{value}",
      justice: "公正使 Entropy 增长速度 ÷{value}",
      revolution: "革新使 CE 萃取量 ×{value}",
      contributions: {
        hope: "希望 {amount}：缓和兼并的力度 {contribution}",
        conquest: "征服 {amount}：压制 CP 获取的力度 {contribution}",
        consensus: "共识 {amount}：使 Shielding Efficiency 提升至 ×{contribution}",
        law: "法度 {amount}：抑制 DC 价格增长的力度 {contribution}",
        shackle: "桎梏 {amount}：加剧兼并的力度 {contribution}",
        freedom: "自由 {amount}：使前四个 DE Softcap Efficiency 提升至 ×{contribution}",
        enlightenment: "启蒙 {amount}：推动 CP 获取的力度 {contribution}",
        war: "战火 {amount}：加剧 DC 价格增长的力度 {contribution}",
        peace: "和平 {amount}：使概念生产速度提升至 ×{contribution}",
      },
    },
  },
  ...
  crushMilestones: {
    ...
    placeholderNote: "全局梦境水晶 ×10，以及 Crush I 至 Crush VI 的效果已经实装；后续里程碑效果暂为占位。",
    ...
    items: {
      ...
      "4": {
        title: "Crush IV",
        effect: "桎梏转化为自由：不再加剧兼并，并根据自由数量降低前四个 DE Softcap 的 strength；所有层的 Entropy 增长速度 ×10。",
      },
      "5": {
        title: "Crush V",
        effect: "概念水晶解锁可独立切断生产的信念、公正与革新，但第 3、4 阶梦境水晶无法生产。",
      },
      "6": {
        title: "Crush VI",
        effect: "角色系统与所有角色初始解锁；战火转化为和平：不再加剧 DC 价格增长，并逐渐提升概念生产速度；所有层的有效混沌指数 ×500。",
      },
      "7": {
        title: "Crush VII",
  ...
  crush: {
    ...
    dialog: {
      ...
      placeholderWarning: "里程碑会正常记录；Crush I 至 Crush VI 效果已经实装，后续里程碑的独立效果暂为占位。",
      cancel: "取消",
    },
  },
  realityMilestones: {
  ...
  currentStratum: {
    ...
    conceptConflict: {
      ...
      strengthMultiplier: "概念冲突结算后，当前梦能为冲突边界的 {value}。",
    },
    shielding: {
      title: "Shielding",
